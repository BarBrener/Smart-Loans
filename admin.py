...
async def delete_loan(user: user_dependency, db: db_dependency, loan_id: int = Path(gt=0)):
    ...
    # Fetch user and account associated with user_id
    loan_to_delete = db.query(Loans).filter(Loans.loan_id == loan_id).first()
...
@router.get("/missed-loans", status_code=status.HTTP_200_OK)
...
@router.post("/punish-missed-payments", status_code=status.HTTP_200_OK)
async def punish_missed_payments(user: user_dependency, db: db_dependency):
    ...
    for loan in overdue_loans:
        ...
        current_user_profile = db.query(Users).filter(
            Users.id == current_account.user_id).first() if current_account else None
        ...
        # ✅ Transfer ETH from borrower to admin using secure transfer
        transfer_request = TransferRequest(
        ...
        # ✅ Save original remaining balance before resetting
        original_due_before = loan.remaining_balance
        ...
        punished_loans_list.append({
            ...
            "original_due": original_due_before,
            "penalty": penalty,
...
async def secure_transfer_to_admin(user: user_dependency, db: db_dependency, transfer_request: TransferRequest):
    ...
    sender_account = db.query(Account).filter(Account.user_id == user.get("id")).first()
    ...
    sender_account.balance = get_account_balance(user.get("public_key"))

    # ✅ Prepare blockchain transaction (Transfer from USER to ADMIN)
    transaction = {
        'from': user.get("public_key"),  # ✅ Sender's Ethereum address
